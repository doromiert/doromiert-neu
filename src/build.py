...
def parse_frontmatter(text):
    if text.startswith("---"):
        parts = text.split("---", 2)
        if len(parts) >= 3:
            meta = {l.split(":", 1)[0].strip(): l.split(":", 1)[1].strip() for l in parts[1].strip().splitlines() if ":" in l}
...
def article_page(title, body_html, section_id):
    ...
    return (
        ...
        f'<style>{(PROJ / "src" / "base.css").read_text() if (PROJ / "src" / "base.css").exists() else ""}</style>'
        ...
        f'view-transition-name: page-main}}'
        f'</style>'
        f'</head><body>'
...
def compile_page(html):
    ...
    html = html.replace("</style>", generate_icon_css("/icons.svg") + "</style>", 1)
    return html

def build_lib():
    src = DATA / "lib"
    if not src.exists(): return ""
    out = DIST / "lib"; out.mkdir(parents=True, exist_ok=True)
    cards = []
    for f in sorted(src.glob("*.md")):
        meta, body = parse_frontmatter(f.read_text())
        title = meta.get("title", f.stem); icon = meta.get("icon", "docs")
        href = meta.get("url", f"/lib/{f.stem}.html")
        cards.append(f'<a class="card" href="{href}"><nz-icon name="{icon}" size="32"></nz-icon><b>{title}</b></a>')
        if not meta.get("url"):
            body_html = f'<div class="art-header"><nz-icon name="{icon}" size="64"></nz-icon><b>{title}</b></div><article>{mdlib.markdown(body)}</article>'
...
def build_blog():
    src = DATA / "blog"
    if not src.exists(): return ""
    out = DIST / "blog"; out.mkdir(parents=True, exist_ok=True)
    ...
    for i, (slug, meta, body) in enumerate(posts):
        ...
        body_html = f'<div class="art-header"><b style="font-size:18px">{date}</b></div><article>{mdlib.markdown(body)}</article>'
        (out / f"{slug}.html").write_text(compile_page(article_page(date, body_html, "blog")))
    return f'<div class="blog-body">{mdlib.markdown(posts[0][2])}</div>'

def build_category(sid):
    src = DATA / sid
    if not src.exists(): return ""
    out = DIST / sid; out.mkdir(parents=True, exist_ok=True)
    items_html = []
    for f in sorted(src.glob("*.md")):
        ...
        title = meta.get("title", f.stem); icon = meta.get("icon", "docs")
        body_html = f'<div class="art-header"><nz-icon name="{icon}" size="64"></nz-icon><b>{title}</b></div><article>{mdlib.markdown(body)}</article>'
        (out / f"{f.stem}.html").write_text(compile_page(article_page(title, body_html, sid)))
        items_html.append(f'<a class="cat-item" href="/{sid}/{f.stem}.html"><span>{title}</span></a>')
    return f'<div class="cat-list">{"".join(items_html)}</div>'

def inject_section(html, sid, content):
    return re.sub(rf'id="{sid}"[^>]*>(.*?)</nz-section>', lambda m: m.group(0).replace("></nz-section>", f">{content}</nz-section>", 1), html, flags=re.DOTALL)
